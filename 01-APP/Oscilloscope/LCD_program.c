...
/*
void LCD_VoidWriteNumberFromDecimalToAsci(signed long int  Copy_Number)
{
	signed long int Temp1=1;
	unsigned char Iterator=0;

	if(					Copy_Number					)
	{
		if(				Copy_Number			<		0				) {printf("-");Copy_Number*=-1;}
		while(		Copy_Number		!=		0				)
		{
			Temp1=10*Temp1+(Copy_Number%10);
			Copy_Number/=10;
			Iterator++;
		}
